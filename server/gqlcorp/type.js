...
const typeData = () => {
  ...
  for (const [key, value] of Object.entries(data.allTables)) {
    // key is col name, val is array holding objs of column data
...
const dataTupleMaker = (object) => {
  ...
  for (const [tableName, colData] of tupColNames) {
    // key is col name, val is array holding multiple objs of column data // reducing array of objs to obj of col names and types
...
///////////////////////////////////////////////////////////////////////////////////////
const fkTupleMaker = (array) => {
  // array of Foreign Keys, elem is object
  const tupArr = [];
  array.forEach((fkObj) => {
    if (
      !tupArr.length ||
      tupArr[tupArr.length - 1][0] !== fkObj.foreign_table
    ) {
      //array not empty, last tuple in array's name is not yet made:
      ...
      // fTable already exists in fkArray
...
const atd = [
  [
    'planets', // all table data / data tuples
    {
      _id: 'integer',
...
const isJoinTable = (fKeyCount, pKeyCount, fktObject, atdObject) => {
  //filling non & joinTables
  for (const key in pKeyCount) {
    if (fKeyCount[key] + 1 === pKeyCount[key]) {
      // loop through fkey table & find if key is in pKey table AND # corresponds to # in pKey => IS A JOIN TABLE // never > but is === always going to be true? test later shit to do rn
...
const nonAndJoinTables = (numFKeys, numAllKeys, fKeysObject, tablesObj) => {
  ...
  for (const tableName in numAllKeys) {
    ...
    if (numFKeys[tableName] + 1 === numAllKeys[tableName])
      jTable[tableName] = tablesObj[tableName];
...
const typeCreator = (nonJoinTables, fktObj) => {
  ...
  for (const key in nonJoinTables) {
    // key: people / starship spec
    ...
    if (!fktObj[key]) {
      ...
      for (const prop in nonJoinTables[key]) {
        //prop: key within the people obj / hyperdrive
...
const otherKeyFinder = (typeObject, fKTAO) => {
  //fkTAO = foreign key table object
  // take type obj keys
...
const terrified = (nonJoinTables, fktObj, nullable) => {
  ...
  for (const njtCol in nonJoinTables) {
    //type object name i.e. planets, species, films
    for (const fktCol in fktObj) {
      //iterate through fktObj fktCol i.e. people, peope_in_films
      // console.log(`1082`)
      for (const col in fktObj[fktCol]) {
        //iterate through nested obj
        // console.log(`1083`)
