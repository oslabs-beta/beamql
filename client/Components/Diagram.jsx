    ...
    <div data-testid="NodeTest" style={customNodeStyles}>
      ...
      {data.pk ? (
        <Handle
          ...
          position="left"
          key={keyNum++}
          style={{ top: '56px', borderRadius: 5 }}
        />
      ) : (
        ''
      )}
      {data.columns.map((el) => (
        <div key={keyNum++}>{el}</div>
      ))}
      <div>
        {data.fks.map((el) => (
          <Handle
            ...
            key={`${keyNum++}`}
            id={`${el.pos}`}
            style={{ top: `${56 + (el.pos - 1) * 23}px`, borderRadius: 5 }}
          />
        ))}
      </div>
    </div>
  ...
  for (let [key, val] of Object.entries(data.allTables)) {
    ...
    val.forEach((el) => {
      for (let j = 0; j < data.foreignKeys.length; j++) {
        let fkObj = data.foreignKeys[j];
        if (
          fkObj.fk_columns === el.column_name &&
          fkObj.foreign_table === key
        ) {
          newNode.data.fks.push({ ...fkObj, pos: el.ordinal_position });
        }
      }
  ...
  return (
    ...
    >
      <ReactFlow elements={elements} nodeTypes={nodeTypes}>
        <Background id="erdbackground" variant="dots" />
